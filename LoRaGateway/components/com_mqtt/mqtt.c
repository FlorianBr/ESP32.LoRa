...
#define MQTT_ID "LoRaGW"         // Start of the base ID
  ...
  switch ((esp_mqtt_event_id_t)event_id) {
    ...
    case MQTT_EVENT_DATA:
      ...
      if ((BaseTopic_len > 0) && (BaseTopic_len < event->topic_len)) {
        ...
        ESP_LOGD(TAG, "Enqueueing Rx message: Topic='%s' with %d bytes data", RxMsg.SubTopic, strlen(RxMsg.Payload));
...
esp_err_t MQTT_Init(void) {
  ...
  snprintf(&BaseTopic[0], MAX_BASE_LENGTH, "%s_%02x%02x%02x%02x%02x%02x", MQTT_ID, Mac[0], Mac[1], Mac[2], Mac[3],
           Mac[4], Mac[5]);
